...
def language(request):
    try:
        if request.user.is_authenticated:
            ...
            try:
                language = Language.objects.get(language_id=_language_id(request=request))  # Get cart using the _cart_id
            except Language.DoesNotExist:
                language = Language.objects.create(language_id=_language_id(request=request))
